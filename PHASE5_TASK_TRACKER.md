...
- [ ] **Task #21.1** - 实现 TaskScheduler（3 小时）
  - 文件: `python/src/scheduler/task_scheduler.py`
  - 内容: APScheduler 封装，任务 CRUD，持久化
  - 验收: 可添加/删除/暂停/恢复任务

- [ ] **Task #21.2** - 导出调度模块（0.2 小时）
  - 文件: `python/src/scheduler/__init__.py`
  - 内容: 导出 TaskScheduler 类
  - 验收: 可导入
...
**Day 3 验收**: ✅ 调度器可按 Cron 表达式执行任务

**Week 1 总验收**: ✅ 通知和调度基础框架完成
